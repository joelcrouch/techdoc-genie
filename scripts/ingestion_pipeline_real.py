...
# #!/usr/bin/env python3
# """
# Build a disk-backed vector store for all PostgreSQL HTML docs,
# with directory names reflecting the chunking strategy.
...
#     # Step 1: Load all PostgreSQL HTML files
#     loader = DocumentLoader(data_dir="data/raw/postgresql", doc_format="html")
#     documents = loader.load_and_prepare()
#     logger.info(f"✅ Loaded {len(documents)} documents")

#     # Step 2: Chunk documents
#     chunker = DocumentChunker(chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP)
#     chunks = chunker.chunk_documents(documents, strategy=CHUNK_STRATEGY)
#     logger.info(f"✅ Created {len(chunks)} chunks with strategy {CHUNK_STRATEGY}")
...
#     for doc in results:
#         logger.info(f"- {doc.metadata.get('filename', 'unknown')} | {doc.page_content[:100]}...")
