...
def run_rag_evaluation_parallel(filter_provider: Optional[str] = None, filter_model: Optional[str] = None):
    ...
    answer quality against ground truth with checkpointing. Can be filtered
    to run against a specific LLM provider and model.
    """
    ...
    # Filter LLM configurations if provider and/or model are specified
    if filter_provider:
        original_llm_configs_count = len(llm_configs)
        llm_configs = [
            config for config in llm_configs
            if config.get("provider") == filter_provider
        ]
        if filter_model:
            llm_configs = [
                config for config in llm_configs
                if config.get("model_id") == filter_model
            ]
        
        if not llm_configs:
            logger.error(f"No LLM configurations found matching provider '{filter_provider}' and model '{filter_model}' in {EVALUATION_CONFIGS_PATH}.")
            sys.exit(1)
        else:
            logger.info(f"Filtered LLM configurations from {original_llm_configs_count} to {len(llm_configs)}: {llm_configs}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run parallel RAG answer quality evaluation.")
    parser.add_argument(
        "--provider",
        type=str,
